    ...
    try {
        ...
        if (!user) {
            ...
            if (!setting.isReadyAward) {
                ...
                if(userText.toLowerCase() === setting.correct_answer){
...
async function sendMessageToAllUsers() {
    try {
        ...
        // if (setting.membersCnt < 2) {
            ...
            setting.correct_answer = correct_answer.toLowerCase();
            await setting.save();
...
const scheduleRandomInterval = () => {
    ...
    // const randomInterval = standardTimeUnit;
    console.log(`Next execution in ${randomInterval / 1000} seconds`);
    setTimeout(executeTask, randomInterval);
};
